...

{
    ...
    public async Task<IEnumerable<TDalEntity>> GetAllIncludingAsync(params Expression<Func<TDalEntity, object>>[] includeProperties)
    {
        IQueryable<TDalEntity> query = RepoDbContext.Set<TDalEntity>();
    ...
    public async Task<TDalEntity?> FirstOrDefaultAsyncInclude(TKey id, TKey userId = default, bool noTracking = true, params Expression<Func<TDalEntity, object>>[] includeProperties)
    {
        var query = CreateQuery(userId, noTracking).Where(e => e.Id.Equals(id));
        
        var propertyNames = includeProperties.Select(expr =>
        {
